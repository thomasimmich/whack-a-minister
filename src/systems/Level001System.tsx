...
import { CoinsFacet, ScoreFacet, LevelFacet, HitableFacet, GameStateFacet, GameStates, NameFacet, ImageFacet, CurrentLevelFacet } from "../app/GameFacets";
...
export const InitSystem = () => {
    ...
    useEffect(() => {
      console.log('level001 init');
      ecs.engine.clear();
      
      // Current Level
      const currentLevelEntity = new Entity();
      ecs.engine.addEntity(currentLevelEntity);
      currentLevelEntity.addComponent(new CurrentLevelFacet({ currentLevelValue: 1 }));
      ...
      level1Entity.addComponent(new NameFacet({ name: "Level 002" }));
      level1Entity.addComponent(new ImageFacet({ src: BASE_ASSET_URL + '/images/menu/card-2.png' }));
